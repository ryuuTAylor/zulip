...
from zerver.lib.scheduled_messages import compute_next_delivery
from zerver.lib.test_classes import ZulipTestCase
from zerver.lib.test_helpers import most_recent_message
from zerver.lib.timestamp import timestamp_to_datetime
from zerver.models import Attachment, Message, Recipient, ScheduledMessage, Stream, UserMessage
...
class ScheduledMessageTest(ZulipTestCase):
    ...
    def test_schedule_message(self) -> None:
        ...
        self.assertEqual(scheduled_message.next_delivery, scheduled_message.scheduled_timestamp)
    ...
    def test_successful_deliver_daily_recurring_scheduled_message(self) -> None:
        with time_machine.travel(NOW, tick=False):
    ...
    def test_successful_deliver_weekly_recurring_scheduled_message(self) -> None:
        with time_machine.travel(NOW, tick=False):
            scheduled_message = self.create_recurring_scheduled_message(
                next_delivery=datetime(2026, 1, 7, 9, 59, 0, tzinfo=UTC),
                recurrence_type=ScheduledMessage.WEEKLY,
                recurrence_days=[2],
                scheduled_time=datetime_time(9, 0),
            )
    ...
    def test_successful_deliver_monthly_recurring_scheduled_message(self) -> None:
        with time_machine.travel(NOW, tick=False):
            scheduled_message = self.create_recurring_scheduled_message(
                next_delivery=datetime(2026, 1, 7, 9, 59, 0, tzinfo=UTC),
                recurrence_type=ScheduledMessage.MONTHLY,
                recurrence_days={"type": "calendar_day", "day": 15},
                scheduled_time=datetime_time(9, 0),
            )
    ...
    def test_worker_skips_future_recurring_scheduled_message(self) -> None:
        with time_machine.travel(NOW, tick=False):
            self.create_recurring_scheduled_message(
                next_delivery=datetime(2026, 1, 7, 11, 0, 0, tzinfo=UTC),
                recurrence_type=ScheduledMessage.DAILY,
                recurrence_days=[],
                scheduled_time=datetime_time(11, 0),
            )
...
class BatchScheduledMessageRecurrenceTest(ZulipTestCase):
    """Tests for the recurrence fields added to the batch scheduled message endpoint."""

    def post_batch(self, payload: dict[str, Any]) -> "TestHttpResponse":
        self.login("hamlet")
        return self.client_post("/json/batch_scheduled_messages", payload)

    def make_stream_dest(
        self, stream_name: str = "Verona", topic: str = "test topic"
    ) -> dict[str, Any]:
        return {"type": "stream", "stream_id": self.get_stream_id(stream_name), "topic": topic}

    @time_machine.travel(NOW, tick=False)
    def test_one_time_batch_still_works(self) -> None:
        """Existing one-time batch path is unaffected by the new optional params."""
        dest = self.make_stream_dest()
        future_ts = int((NOW + timedelta(minutes=10)).timestamp())
        result = self.post_batch(
            {
                "content": "hello",
                "destinations": orjson.dumps([dest]).decode(),
                "scheduled_delivery_timestamp": orjson.dumps(future_ts).decode(),
            }
        )
        self.assert_json_success(result)
        data = result.json()
        self.assertIn("batch_group_id", data)
        self.assertEqual(data["count"], 1)

    @time_machine.travel(NOW, tick=False)
    def test_recurring_batch_daily(self) -> None:
        """Batch endpoint accepts recurrence fields and creates recurring rows."""
        dest = self.make_stream_dest()
        result = self.post_batch(
            {
        ...
        msg = ScheduledMessage.objects.get(id=data["scheduled_message_ids"][0])
        self.assertEqual(msg.recurrence_type, ScheduledMessage.DAILY)
        self.assertEqual(msg.recurrence_days, [])
        self.assertIsNotNone(msg.scheduled_time)
        self.assertIsNotNone(msg.batch_group_id)

    @time_machine.travel(NOW, tick=False)
    def test_recurring_batch_multiple_destinations(self) -> None:
        """Each destination gets its own recurring row, all sharing a batch_group_id."""
        dest1 = self.make_stream_dest("Verona", "topic-a")
        dest2 = self.make_stream_dest("Verona", "topic-b")
        result = self.post_batch(
            {
                "content": "weekly report",
                "destinations": orjson.dumps([dest1, dest2]).decode(),
                "recurrence_type": "weekly",
                "recurrence_days": orjson.dumps([0, 4]).decode(),
    ...
    @time_machine.travel(NOW, tick=False)
    def test_recurring_batch_rejects_timestamp_with_recurrence(self) -> None:
        """scheduled_delivery_timestamp must not be combined with recurrence fields."""
        dest = self.make_stream_dest()
        future_ts = int((NOW + timedelta(minutes=10)).timestamp())
        result = self.post_batch(
            {
        ...
        self.assert_json_error(
            result,
            "scheduled_delivery_timestamp is only supported for one-time scheduled messages.",
        )

    @time_machine.travel(NOW, tick=False)
    def test_recurring_batch_requires_scheduled_time(self) -> None:
        """scheduled_time is required when recurrence_type is given."""
        dest = self.make_stream_dest()
        result = self.post_batch(
            {
                "content": "oops",
        ...
        self.assert_json_error(
            result, "scheduled_time is required for recurring scheduled messages."
        )
